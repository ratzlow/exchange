...
import scala.Some
import org.exchange.matching.auction.AuctionException
import org.exchange.matching.auction.AuctionMatch
import org.exchange.matching.engine.MatchResult
import org.exchange.matching.auction.AuctionConditions
...
class Auction(orderbook: Orderbook) {
  ...
  private def isLimitOrder(order: Order): Boolean =
    order.orderType == Limit || order.orderType == StopLimit
