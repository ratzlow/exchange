...
class AuctionMatchTest extends FunSuite with GivenWhenThen {
  ...
  test("4) There are several possible limits and there is both an ask and bid surplus") {
    ...
    expectResult(199) {
      Auction(orderbook).deriveAuctionConditions(Option(199)).auctionPrice.get
    }
    expectResult(199) {
      Auction(orderbook).deriveAuctionConditions(Option(200)).auctionPrice.get
    }
    expectResult(202) {
      Auction(orderbook).deriveAuctionConditions(Option(201)).auctionPrice.get
    }
    expectResult(202) {
      Auction(orderbook).deriveAuctionConditions(Option(202)).auctionPrice.get
    }
  }
